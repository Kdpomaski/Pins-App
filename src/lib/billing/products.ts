...
/** Free users see map history within this many days when paywall enforce is on. */
 ...
 * Free keeps: log / reminders / 1 protocol / basic recon / basic rotation / local storage.
 * Pro unlocks: unlimited protocols, full map history, export/PDF, cloud sync,
...
export function filterMapHistoryLogs<T extends { time?: string; timestamp?: string }>(
  logs: T[],
  opts: { isPro: boolean; enforce: boolean; now?: number },
): T[] {
  if (opts.isPro || !opts.enforce) return logs;
  const now = opts.now ?? Date.now();
  const cutoff = now - FREE_MAP_HISTORY_DAYS * 24 * 60 * 60 * 1000;
